...
int advance(char *lp, char *ep) {  char *curlp;  int i;
  for (;;) {
    switch (*ep++) {
      ...
      case CDOT|STAR:  curlp = lp;  while (*lp++) { } star(lp,ep);
      case CCHR|STAR:  curlp = lp;  while (*lp++ == *ep) { }  ++ep;  star(lp,ep);
      case CCL|STAR:
      case NCCL|STAR:  curlp = lp;  while (cclass(ep, *lp++, ep[-1] == (CCL|STAR))) { }  ep += *ep; star(lp,ep);
      default: error(Q);
    }
  }
}

int star(char*lp, char*ep) {
  char*curlp;
...
void error(char *s) {
  int c,  wrapp = 0,  listf = 0,  listn = 0; puts_(s);
